...
def compute_case1_metrics(csv_path: str, theory_fn: Callable) -> Dict[str, float]:
    """计算Case1的定量指标"""
    df = pd.read_csv(csv_path)
    
    # 提取数据
    p_data = df['p_0'].values  # 或对应的p列名
    x_data = df['x_0'].values  # 或对应的x列名
    residual = df['res'].values
    arc_err = df['arc_err'].values
    
    # 计算指标
    max_residual = np.max(np.abs(residual))
    
    # 理论解MAE
    x_stable, x_unstable = theory_fn(p_data)
    mae = np.mean(np.minimum(
        np.abs(x_data - x_stable),
        np.abs(x_data - x_unstable)
    ))
    
    mean_arc_error = np.mean(arc_err)
    
    # 分叉点检测
    zero_crossings = np.where(np.diff(np.sign(x_data)))[0]
    if len(zero_crossings) > 0:
        bifurcation_p = p_data[zero_crossings[0]]
        bifurcation_error = abs(bifurcation_p - 0.0)  # 理论值为0
    else:
        bifurcation_error = np.nan
    
    return {
        "max_residual": max_residual,
        "mae_theory": mae,
        "mean_arc_error": mean_arc_error,
